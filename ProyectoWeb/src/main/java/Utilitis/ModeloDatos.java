...
public class ModeloDatos {
    ...
    public HashMap<String, Pelicula> getPeliculas() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                sinopsis = rs.getString(2);
                paginaoficial = rs.getString(3);
                titulooriginal = rs.getString(4);
                genero = rs.getString(5);
                nacionalidad = rs.getString(6);
                duracion = rs.getInt(7);
                ano = rs.getInt(8);
                distribuidora = rs.getString(9);
    ...
    public boolean existePelicula(String nombre){
        return getPeliculas().containsKey(nombre);
    }
    ...
    public void updatePelicula(Pelicula pelicula) {
        String query = "UPDATE PELICULA SET SINOPSIS=?, PAGINAOFICIAL=?, TITULOORIGINAL=?, GENERO=?, NACIONALIDAD=?,"
                + " DURACION=?, AÑO=?, DISTRIBUIDORA=?, DIRECTOR=?, CLASIFICACION=?, PORTADA=? WHERE NOMBRE=?";
        try {
            PreparedStatement queryCompleta = conexion.prepareStatement(query);
            queryCompleta.setString(1, pelicula.getSinopsis());
            queryCompleta.setString(2, pelicula.getPaginaoficial());
            queryCompleta.setString(3, pelicula.getTitulooriginal());
            queryCompleta.setString(4, pelicula.getGenero());
            queryCompleta.setString(5, pelicula.getNacionalidad());
            queryCompleta.setInt(6, pelicula.getDuracion());
            queryCompleta.setInt(7, pelicula.getAno());
            queryCompleta.setString(8, pelicula.getDistribuidora());
            queryCompleta.setString(9, pelicula.getDirector());
            queryCompleta.setString(10, pelicula.getClasificacion());
            queryCompleta.setString(11, pelicula.getImagen());
            queryCompleta.setString(12, pelicula.getNombre());
